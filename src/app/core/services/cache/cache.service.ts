...
import store from './local-storage-cache.service';
import { CacheConfig, CacheNotifyResult, CacheNotifyType, ICache } from './cache.type';
import { SafeAny } from '@shared/types/yacommon';
import { httpSrv } from '../http/http.service';
...
class CacheService {
	...
	private deepGet(obj: SafeAny, path: string[], defaultValue?: SafeAny): SafeAny {
	...
	private loadMeta(): void {
		const ret = store.get(this.cog.meta_key!);
		if (ret && ret.v) {
	...
	private saveMeta(): void {
		...
		store.set(this.cog.meta_key!, { v: metaData, e: 0 });
	}
	 ...
	 * - `set('data/1', this.http.get('data/1'), { expire: 10 }).subscribe()`
	 */
	set(key: string, data: Observable<SafeAny>, options?: { type?: 's'; expire?: number; emitNotify?: boolean }): Observable<SafeAny>;
	/**
	...
	set(
		key: string,
		data: SafeAny | Observable<SafeAny>,
		options: {
			/** 存储类型，'m' 表示内存，'s' 表示持久 */
			type?: 'm' | 's';
			/**
		...
		if (options.expire) {
			e = dayjs(new Date()).add(options.expire, 'second').valueOf();
		}
	...
	private save(type: 'm' | 's', key: string, value: ICache, emitNotify = true): void {
		if (type === 'm') {
			this.memory.set(key, value);
		} else {
			store.set(this.cog.prefix + key, value);
			this.pushMeta(key);
		}
		if (emitNotify) {
	...
	): Observable<SafeAny> | SafeAny {
		const isPromise = options.mode !== 'none' && this.cog.mode === 'promise';
		const value = this.memory.has(key) ? (this.memory.get(key) as ICache) : store.get(this.cog.prefix + key);
		if (!value || (value.e && value.e > 0 && value.e < new Date().valueOf())) {
			if (isPromise) {
				return (this.cog.request ? this.cog.request(key) : httpSrv.sendRequest({ router: key, type: 'get' })).pipe(
					map((ret: SafeAny) => this.deepGet(ret, this.cog.reName as string[], null)),
					tap(v =>
						this.set(key, v, {
							type: options.type as SafeAny,
	 ...
	 * 获取缓存，若不存在则设置持久化缓存 `Observable` 对象
	 */
	tryGet(key: string, data: Observable<SafeAny>, options?: { type?: 's'; expire?: number; emitNotify?: boolean }): Observable<SafeAny>;
	/**
	 * 获取缓存，若不存在则设置持久化缓存基础对象
	 */
	tryGet(key: string, data: unknown, options?: { type?: 's'; expire?: number; emitNotify?: boolean }): SafeAny;
	/**
	 * 获取缓存，若不存在则设置指定缓存类型进行缓存对象
	 */
	tryGet(key: string, data: unknown, options: { type: 'm' | 's'; expire?: number; emitNotify?: boolean }): SafeAny;
	...
	tryGet(
		...
		if (ret === null) {
			if (!(data instanceof Observable)) {
				this.set(key, data, options as SafeAny);
				return data;
			}

			return this.set(key, data as Observable<SafeAny>, options as SafeAny);
		}
		return of(ret);
	}
	...
	private _remove(key: string, needNotify: boolean): void {
		...
		store.remove(this.cog.prefix + key);
		this.removeMeta(key);
	}
	...
	clear(): void {
		...
		this.memory.clear();
		this.meta.forEach(key => store.remove(this.cog.prefix + key));
	}
